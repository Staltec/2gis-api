...
function doRequest (namespace, options, onRequestDataCallback) {
    ...
    exports.getFromCache (requestPath, function(err, data) {

        if(!err && data && _.isPlainObject(data)) {
            onRequestDataCallback(null, data);
        } else {
            doHttpRequest (requestObj, function(err, data){
                if(!err) {
                    exports.pushToCache(requestPath, data);
                }

                onRequestDataCallback (err, data);
            });
        }
